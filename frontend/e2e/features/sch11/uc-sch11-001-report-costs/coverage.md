...
2. **Read-only OMITS rather than disables.** Legacy disabled the six Add fields and every row control
   ...
   all when `editable` is false (DIV-3).
3. **One Check Status button, not two** (legacy had both `:191` and `:426` — DIV-4).
4. **The per-field "original value" indicators are gone.** All six legacy row fields carried an
   ...
   app has no counterpart, and the API exposes no prior value to render one from (DIV-6). Found on
   2026-08-10 by checking whether Schedule 1's DIV-5 also applies here — it does, same root cause, so it
   should ride that ticket rather than a new one. Out of reach for this UC's scenarios, which all write
   against Draft.
...
| Per-row optimistic lock — stale `revisionCount` → 409 | **no legacy slice** (new-app concurrency) | `Schedule11Service.editLocation` stale check; `revisionCount` read at `startEdit` (index.tsx:639) | — | deferred | Confirmed live: `PUT` with a stale token → 409 "This schedule was changed by another user. Please reload and try again." Needs two concurrent sessions to drive from the UI — GAP-3 (also SPEC-1) |
| Duplicate biogeo/location key → 409 | **no legacy slice**; `Schedule11Api.addLocation` javadoc | `Schedule11Service.addLocation` duplicate check | — | deferred | GAP-4 (also SPEC-1) |
| Unresolvable BEC id → 400 `invalidBiogeoCode` | **no legacy slice**; `InvalidBiogeoCodeException` | `Schedule11Service` biogeo resolution | — | not-applicable (UI) | Forced selection means the UI can only ever submit an id it just received from the catalogue; reachable only via a direct API call or a dangling FK row. GAP-4 |
...
| Column sorting (3-state, per column) | **no legacy slice** — new-app behaviour | `handleSort` / `COLUMNS` (index.tsx:524, 258) | — | deferred | The app ADDS a NONE state legacy lacked (PrimeFaces only toggled asc/desc). Presentational, no persistence. GAP-5 (also SPEC-2) |
